...
export default function CourseDetail() {
  ...
  const handleEnroll = () => {
    ...
    enroll(course.id);
    toast({ title: "Berhasil enroll!", description: `Kamu kini terdaftar di ${course.title}` });
  };
          ...
          <div className="grid gap-10 md:grid-cols-[1fr,400px]">
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-6 text-sm text-muted-foreground">
                <div className="flex items-center gap-1.5"><Star className="h-4 w-4 fill-warning text-warning" /> {course.rating} rating</div>
                <div className="flex items-center gap-1.5"><Users className="h-4 w-4" /> {course.students.toLocaleString()} siswa</div>
                <div className="flex items-center gap-1.5"><Clock className="h-4 w-4" /> {course.lessons.length} materi</div>
              </div>
